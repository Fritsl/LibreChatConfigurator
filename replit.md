...
**UI Organization**: The UI/Visibility tab is the first tab in the interface, consolidating app title, welcome message, footer text, help URL, and all interface visibility settings in one central location. An informational alert at the top of this tab explains how to hide the deprecated "Plugins" menu using the Model Select Interface and Visible Endpoints settings. Technical metadata is displayed in clickable popovers (click the info icon) showing the exact env var/YAML path and target file for each configuration field.

**Hiding Deprecated Plugins Menu (v0.8.0-rc4 Workaround)**: LibreChat RC4's frontend hard-codes the deprecated Plugins menu to always show, regardless of ENDPOINTS env var. The proper workaround uses two settings in the UI/Visibility tab: (1) **Enable Model Select Interface** (interface.modelSelect: true) and (2) **Visible Endpoints (UI)** (modelSpecs.addedEndpoints) set to exclude gptPlugins. Both settings are enabled by default with correct values to hide the deprecated menu.

**Enabled Endpoints Configuration**: The Server Configuration tab includes an "Enabled Endpoints" setting that controls which AI providers the backend API serves via the ENDPOINTS environment variable. By default, it includes openAI, anthropic, google, azureOpenAI, and agents, but excludes gptPlugins (deprecated in RC4). Note: This controls the API only; the UI visibility requires the modelSpecs.addedEndpoints setting described above.
