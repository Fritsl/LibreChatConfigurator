 ...
 */
function detectPersistentPaths(config: any): Array<{
  fieldId: string;
  containerPath: string;
  hostPath: string;
  description: string;
}> {
  const persistentPaths: Array<{
    fieldId: string;
    containerPath: string;
    hostPath: string;
    description: string;
  ...
  for (const field of FIELD_REGISTRY) {
    const metadata = field.persistenceMetadata;
    if (!metadata || !metadata.requiresPersistence) continue;

    // Check conditional logic (e.g., only persist if fileStrategy is 'local')
    if (metadata.conditionalOn) {
      const condField = metadata.conditionalOn.field;
      const condValues = metadata.conditionalOn.values;
      // Treat undefined as empty string (default value)
      const actualValue = config[condField] ?? '';
      
      // Skip if condition not met
      if (!condValues.includes(actualValue)) {
        console.log(`⏭️  [PERSISTENCE] Skipping ${field.id} - ${condField}=${actualValue || '(empty)'} not in [${condValues.join(', ')}]`);
        continue;
      }
    }

    // Get configured value or use default
    const configuredPath = config[field.id];
    const containerPath = configuredPath || metadata.defaultContainerPath;
    
    // Only add if a path is configured
    if (!containerPath) continue;

    persistentPaths.push({
      fieldId: field.id,
      containerPath,
      hostPath: metadata.defaultHostPath || `./data/${field.id}`,
      description: metadata.dataDescription || field.description || field.id
    });
    
    console.log(`✅ [PERSISTENCE] Detected persistent path: ${field.id} → ${containerPath} (${metadata.dataDescription})`);
  }
