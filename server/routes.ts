...
/**
 * Normalize configuration to ensure legacy fields are migrated to their new locations
 * This fixes dual-placement bugs where fields existed in multiple schema locations
 */
function normalizeConfiguration(config: any): any {
  const normalized = { ...config };
  
  // Migrate interface.customWelcome to top-level customWelcome (if not already set)
  if (config.interface?.customWelcome && !config.customWelcome) {
    normalized.customWelcome = config.interface.customWelcome;
  }
  
  // Migrate interface.customFooter to top-level customFooter (if not already set)
  if (config.interface?.customFooter && !config.customFooter) {
    normalized.customFooter = config.interface.customFooter;
  }
  
  // Migrate interface.temporaryChatRetention to top-level temporaryChatRetention (if not already set)
  if (config.interface?.temporaryChatRetention && !config.temporaryChatRetention) {
    normalized.temporaryChatRetention = config.interface.temporaryChatRetention;
  }
...
function generateDockerComposeFile(config: any): string {
  // Normalize config to migrate legacy dual-placement fields
...
services:
  ...
  mongodb:
    ...
    environment:
      MONGO_INITDB_ROOT_USERNAME: \${MONGO_ROOT_USERNAME:-${normalizedConfig.mongoRootUsername || 'librechat_admin'}}
      MONGO_INITDB_ROOT_PASSWORD: \${MONGO_ROOT_PASSWORD:-${normalizedConfig.mongoRootPassword || 'librechat_password_change_this'}}
      MONGO_INITDB_DATABASE: \${MONGO_DB_NAME:-${normalizedConfig.mongoDbName || 'librechat'}}
    networks:
      - librechat-network
    ports:
      - "27017:27017"
...
${normalizedConfig.e2bProxyEnabled ? `
  ...
  e2b-proxy:
    ...
    healthcheck:
      ...
      start_period: 10s
` : ''}
  # LibreChat Application${normalizedConfig.interface?.artifacts ? `
  ...
  librechat:
    ...
    depends_on:
      ...
      redis:
        condition: service_started${normalizedConfig.e2bProxyEnabled ? '\n      e2b-proxy:\n        condition: service_started' : ''}
    ports:
      - "\${LIBRECHAT_PORT:-${normalizedConfig.port}}:3080"
    environment:
      # =============================================================================
      # Database Configuration
      # =============================================================================
      MONGO_URI: mongodb://\${MONGO_ROOT_USERNAME:-${normalizedConfig.mongoRootUsername || 'librechat_admin'}}:\${MONGO_ROOT_PASSWORD:-${normalizedConfig.mongoRootPassword || 'librechat_password_change_this'}}@mongodb:27017/\${MONGO_DB_NAME:-${normalizedConfig.mongoDbName || 'librechat'}}?authSource=admin
      REDIS_URI: redis://redis:6379
${normalizedConfig.redisUsername ? `      REDIS_USERNAME: \${REDIS_USERNAME}` : '      # REDIS_USERNAME: ${REDIS_USERNAME}'}
${normalizedConfig.redisPassword ? `      REDIS_PASSWORD: \${REDIS_PASSWORD}` : '      # REDIS_PASSWORD: ${REDIS_PASSWORD}'}
${normalizedConfig.redisKeyPrefix ? `      REDIS_KEY_PREFIX: \${REDIS_KEY_PREFIX}` : '      # REDIS_KEY_PREFIX: ${REDIS_KEY_PREFIX}'}
${normalizedConfig.redisKeyPrefixVar ? `      REDIS_KEY_PREFIX_VAR: \${REDIS_KEY_PREFIX_VAR}` : '      # REDIS_KEY_PREFIX_VAR: ${REDIS_KEY_PREFIX_VAR}'}
${normalizedConfig.redisMaxListeners ? `      REDIS_MAX_LISTENERS: \${REDIS_MAX_LISTENERS}` : '      # REDIS_MAX_LISTENERS: ${REDIS_MAX_LISTENERS}'}
${normalizedConfig.redisPingInterval ? `      REDIS_PING_INTERVAL: \${REDIS_PING_INTERVAL}` : '      # REDIS_PING_INTERVAL: ${REDIS_PING_INTERVAL}'}
${normalizedConfig.redisUseAlternativeDNSLookup !== undefined ? `      REDIS_USE_ALTERNATIVE_DNS_LOOKUP: \${REDIS_USE_ALTERNATIVE_DNS_LOOKUP}` : '      # REDIS_USE_ALTERNATIVE_DNS_LOOKUP: ${REDIS_USE_ALTERNATIVE_DNS_LOOKUP}'}
...
${normalizedConfig.domainClient ? `      DOMAIN_CLIENT: \${DOMAIN_CLIENT}` : '      # DOMAIN_CLIENT: ${DOMAIN_CLIENT}'}
${normalizedConfig.domainServer ? `      DOMAIN_SERVER: \${DOMAIN_SERVER}` : '      # DOMAIN_SERVER: ${DOMAIN_SERVER}'}
${normalizedConfig.appTitle ? `      APP_TITLE: \${APP_TITLE}` : '      # APP_TITLE: ${APP_TITLE}'}
${normalizedConfig.customWelcome ? `      CUSTOM_WELCOME: \${CUSTOM_WELCOME}` : '      # CUSTOM_WELCOME: ${CUSTOM_WELCOME}'}
${normalizedConfig.customFooter ? `      CUSTOM_FOOTER: \${CUSTOM_FOOTER}` : '      # CUSTOM_FOOTER: ${CUSTOM_FOOTER}'}
${normalizedConfig.showBirthdayIcon !== undefined ? `      SHOW_BIRTHDAY_ICON: \${SHOW_BIRTHDAY_ICON}` : '      # SHOW_BIRTHDAY_ICON: ${SHOW_BIRTHDAY_ICON}'}
${normalizedConfig.helpAndFaqUrl ? `      HELP_AND_FAQ_URL: \${HELP_AND_FAQ_URL}` : '      # HELP_AND_FAQ_URL: ${HELP_AND_FAQ_URL}'}
${normalizedConfig.noIndex !== undefined ? `      NO_INDEX: \${NO_INDEX}` : '      # NO_INDEX: ${NO_INDEX}'}
      DOMAIN_CLIENT: \${DOMAIN_CLIENT:-http://localhost:3080}
    volumes:
      - e2b_files:/tmp/e2b-files
    networks:
